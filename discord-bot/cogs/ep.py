"""EP (Engagement Points) management cog — /ep edit."""
...
def _has_ep_permission(interaction: discord.Interaction) -> bool:
    """
    Return True if the user has EP manager permission.

    Passes if the user:
    - is a guild administrator, OR
    - holds the EP_MANAGER_ROLE_ID role, OR
    - holds any role with a higher position than EP_MANAGER_ROLE_ID.
    """
    ...
    if not role_id:
        # Fall back to Manage Server if no role is configured
...
class EP(commands.GroupCog, name="ep"):
    """Manage member Engagement Points."""
    ...
    async def ep_edit(
        ...
        if not record:
            # Attempt to resolve via Roblox API and auto-create if found
