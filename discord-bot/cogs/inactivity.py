...
class EmergencySelectButtons(discord.ui.View):
    """
    Ephemeral buttons to select if inactivity notice is an emergency.
    """
    
    def __init__(self, interaction_data: dict):
        """
        Initialize emergency selection buttons.
        
        Args:
            interaction_data: Dictionary with command parameters
        """
...
class EndEarlyDMButtons(discord.ui.View):
    """
    Buttons for ending inactivity early from DM.
    """
    
    def __init__(self, bot, database, user_id: int, message_id: int, end_date: datetime):
        """
        Initialize end early DM buttons.
        
        Args:
            bot: Bot instance
            database: BotDatabase instance
            user_id: User ID who owns the inactivity notice
            message_id: Message ID of the inactivity notice
    ...
    async def end_early_dm(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Handle end early button click from DM."""
        ...
        try:
            ...
            if cooldown_role:
                ...
                # Schedule cooldown removal (14 days)
            ...
            # Delete the inactivity notice message in channel
...
class InactivityButtons(discord.ui.View):
    """
    Buttons for inactivity notice approval/denial.
    """
    ...
    def __init__(self, bot, database, roblox_api, command_logger, notice_data):
        ...
        Args:
            ...
            notice_data: Dictionary with notice information
        """
        super().__init__(timeout=None)  # Persistent buttons
        self.bot = bot
        self.database = database
...
class Inactivity(commands.Cog):
    """
    Cog for /inactivity-notice command.
    
    Allows users to submit inactivity notices with approval workflow.
    """
    
    def __init__(self, bot: commands.Bot, database, roblox_api, command_logger):
        """
        Initialize Inactivity cog.
    ...
    async def inactivity_notice(
        ...
        """
        Handle /inactivity-notice command.
        ...
        try:
            ...
            # Check if user already has inactivity role
            inactivity_role_id = int(os.getenv('INACTIVITY_ROLE_ID', 0))
            ...
            # Check if user has cooldown role
            cooldown_role_id = int(os.getenv('INACTIVITY_COOLDOWN_ROLE_ID', 0))
            ...
            # If user has cooldown role, ask if this is an emergency
            if has_cooldown_role:
                ...
                # If no selection made (timeout), cancel
                ...
                # If not emergency and they have cooldown, deny
            ...
            # If user already has inactivity role, ask if this is an emergency
            if has_inactivity_role:
                ...
                # If not emergency and they already have role, deny
                if not emergency_view.is_emergency:
                    await interaction.followup.send(
            ...
            # Create embed with emergency status
            ...
            # Update notice data with message ID and store in database
