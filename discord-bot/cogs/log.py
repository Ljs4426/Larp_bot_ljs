...
def _parse_edit_format(text: str) -> List[str]:
    """parse 'orig | new' lines — blank right = remove, |new = add, no pipe = keep"""
    result: List[str] = []
    seen: set = set()
...
class ConfirmLogView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Disable buttons immediately as the interaction response (works for ephemeral messages)
        ...
        # Post event embed to EVENT_LOG_CHANNEL_ID
        ...
        # Post summary to staff log channel
